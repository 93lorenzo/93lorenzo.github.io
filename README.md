
<table>
  <tr>
    <td align="center" valign="top" width="240">
      <img
        src="images/Profile_pic.jpeg"
        alt="Lorenzo Vitali"
        style="display: block; width: 180px; height: 180px; object-fit: cover; object-position: center top; border-radius: 50%;"
      />
    </td>
    <td valign="top">
      <h1>Hi, I'm Lorenzo 👋</h1>
      <p>
        Data Scientist with <strong>7+ years of experience</strong> building end-to-end machine learning products,
        from business requirements and stakeholder alignment to production deployment and model maintenance.<br/>
        I currently work on <strong>Credit and Fraud Monitoring</strong> at Mollie, with a focus on risk detection and loss prevention.
      </p>
      <p>
        <a href="https://www.linkedin.com/in/lorenzo-vitali/">LinkedIn</a> •
        <a href="https://github.com/93lorenzo">GitHub</a>
      </p>
    </td>
  </tr>
</table>

---

## About Me

- I work at the intersection of **machine learning, product thinking, and business impact**.
- I have experience in both **individual contributor** and **lead** roles.
- Core areas: fraud/risk modeling, predictive analytics
---

## Work Experience

### Mollie, Milan  
**Data Scientist - Credit and Fraud Monitoring**  
*June 2023 - Present*

- Develop machine learning models to detect merchants that could negatively impact the business.
- Manage the full lifecycle: stakeholder discussions, model development, presentation, rollout, and maintenance.
- Built key initiatives:
  - Estimates merchant risk from early life stages.
  - Predicts the probability of negative merchant balance for merchants.
  - AI agents to automate tasks.

### DAZN, Milan  
**Data Scientist, Team Lead**  
*June 2019 - June 2023*

- Led end-to-end data science projects from requirements gathering to production.
- Covered data manipulation, model selection, development, and MLOps productionization.
- Managed team operations: mentoring data scientists, roadmap definition, sprint planning, and delivery.
- Delivered multiple data products, including:
  - Demand Prediction, Account's churn, Python internal lib

### Data Reply, Milan  
**Big Data Engineer**  
*November 2018 - June 2019*

- Strengthened software engineering and data engineering best practices.
- Worked on cloud storage, stream processing, and containerization.
- Developed a **Docker Swarm** solution to orchestrate client API request flows end-to-end: ingestion, transformation, and response delivery.

---

## Education

### Sapienza University of Rome, Rome, Italy  
**Master of Science in Artificial Intelligence and Robotics**  
*October 2015 - July 2018*  
- Grade: **110/110**

### University of Kingston, London, England  
**Deep Learning / Computer Vision Researcher - ROVIT (Robot Vision Team)**  
*September 2017 - February 2018*  
- Paper: **Unsupervised Methods for a Personalised Route Recommendation System**

### Universitat Politecnica de Catalunya, Barcelona, Spain  
**Exchange Student (Erasmus+) - Artificial Intelligence Master Courses**  
*February 2016 - June 2016*

### Sapienza University of Rome, Rome, Italy  
**Bachelor of Science in Engineering in Computer Science**  
*October 2012 - October 2015*  
- Grade: **101/110**

---

## External Stuff & Public Speaking

I enjoy sharing practical lessons from production ML, team leadership, and risk modeling.

- **How to prevent frauds in a fintech : static vs dynamic signals** - [Watch Youtube Video (in Italian)](https://www.youtube.com/live/moPIBeNZWew?t=19827s)
- **Data science stuff I wish I knew sooner** - [GitHub Repository](https://github.com/93lorenzo/data_science_stuff_I_wish_I_knew_sooner) where I write data science related articles.

---

## Let's Connect

  <a href="https://www.linkedin.com/in/lorenzo-vitali/">LinkedIn</a> •
  <a href="https://github.com/93lorenzo">GitHub</a>
